...
namespace Fsb\Bundle\ProxyBundle\Command;
...
use Fsb\Bundle\ProxyBundle\Model\User;
use Fsb\Bundle\ProxyBundle\Exception\ExistingUserException;
...
class CreateUserCommand extends ContainerAwareCommand
{
	...
	protected function execute(InputInterface $input, OutputInterface $output)
	{
		...
		$input->isInteractive() ? $output->title('FsbProxyBundle users helper') : $output->newLine();
		...
		$user = new User($username, null, $this->generateSalt(), array('ROLE_USER'));
		...
		$output->table(array(
			...
			array('Salt', $user->getSalt()),
			array('Encoded password', $user->getPassword()),
		));

		$output->success('User successfully ' . ($userExists ? 'replaced' : 'created'));
	}
	...
	private function createUsernameQuestion()
	{
		...
		return $usernameQuestion->setValidator(function ($value) {
			if ('' === trim($value)) {
	...
	{
		return base64_encode($this->getContainer()->get('security.secure_random')->nextBytes($bytes));
	}
}
